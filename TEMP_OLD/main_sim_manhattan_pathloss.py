...
def find_same_street_veh(coords_veh, coords_own):
    ...
    ind_veh_same_coord_dir = np.flatnonzero(
        coords_veh[:, dir_own] == coords_street_own)
    ind_veh_same_street = np.intersect1d(
        ind_veh_same_dir, ind_veh_same_coord_dir)
...
def count_connections(distances, distances_max):
    ...
    counts_range = np.count_nonzero(
        distances_tiled < distances_max_tiled, axis=1)
    return counts_range
...
def plot_scatter_range(coords_own, coords_in_range, coords_out_range, coords_streets_x,
    ...
    # Plot vehicles
    plt.scatter(coords_in_range[:, 0], coords_in_range[
                :, 1], marker='x', label='In range')
    plt.scatter(coords_out_range[:, 0], coords_out_range[
                :, 1], marker='x', label='Out of range')
    plt.scatter(coords_own[0], coords_own[1], c='black',
                marker='o', label='Own vehicle')
...
def main_sim():
    ...
    for i_rep in np.arange(reps):
        ...
        coords_veh_x, coords_streets_x = gen_streets_and_vehicles(lam_s, lam_v, road_len,
                                                                  is_truncated=True)
        coords_veh_x = np.column_stack(
            (coords_veh_x, np.ones(np.shape(coords_veh_x)[0])))
        ...
        ind_other_streets = np.setdiff1d(
            np.arange(np.shape(coords_veh)[0]), ind_same_street)
        ...
        ind_olos = np.setdiff1d(
            np.arange(np.shape(coords_same_street)[0]), ind_los)
        ...
        ind_very_high_pl = np.setdiff1d(
            np.arange(np.shape(coords_other_streets)[0]), ind_nlos)
        ...
        distances_nlos_tx = np.abs(
            coords_nlos[:, dir_own] - coords_own[dir_own])
        ...
        pathlosses[ind_other_streets[ind_very_high_pl]
                   ] = pathlosses_very_high_pl
