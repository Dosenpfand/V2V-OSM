...
def main_sim(network, max_pl=150, debug=False, max_dist_olos_los=300,):
    ...
    print("There are {} vehicles in the simulation".format(count_veh))
    # Find center vehicle
    # time_start = # utils.debug(debug, None, 'Finding center vehicle')
    target_array=[]
    distance_array=[]
    for idx in range(count_veh):
        print("\t Current Run: {}".format(idx))
        idx_center_veh=idx
        idxs_other_vehs=np.where(np.arange(count_veh) != idx_center_veh)[0]
        vehs.add_key('center', idx_center_veh)
        vehs.add_key('other', idxs_other_vehs)
        ...
        target_array.append(len(vehs.get_idxs("olos_los")))
        distance_array.append(d)
        print((target_array, [int(x) for x in distance_array]))
        with open("target_array.p", "wb") as fp:
